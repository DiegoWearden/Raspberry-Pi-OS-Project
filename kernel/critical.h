...
inline void critical(Work work)
{
    ...
    */
    ...
    if (!critical_flag.exchange(true) && critical_depth == 0 && critical_owner == uint32_t(-1)) // first time entry
    {
        ...
        critical_owner = SMP::me();   // set who owns the critical section
    ...
    else if (critical_depth > 0 && critical_owner == SMP::me()) // nested entry
    {
        critical_depth++;
        work();
    ...
    else // waiting for for critical section to be unlocked
    {
        ...
        critical_owner = SMP::me();
        critical_depth++;
        work();
        critical_depth--;
    }

    if (critical_depth == 0 && critical_owner == SMP::me()) // unlock
    {
